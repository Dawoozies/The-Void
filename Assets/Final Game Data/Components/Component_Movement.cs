...
[Serializable]
public class Component_Movement : ScriptableObject, IComponent
{
    //Only one Component per state
    public string controllerName;
    public int stateHash;
    public Component_Movement_Data[] componentData;
    public void AddComponentData(Component_Movement_Data data)
    {
        if (componentData == null)
        {
            componentData = new Component_Movement_Data[1];
            componentData[0] = new Component_Movement_Data();
            data.CopyToNew(componentData[0]);
            EditorUtility.SetDirty(this);
            return;
        }
        Component_Movement_Data[] resizedData = new Component_Movement_Data[componentData.Length + 1];
        for (int i = 0; i < resizedData.Length; i++)
        {
            if(i < componentData.Length)
            {
                resizedData[i] = new Component_Movement_Data();
    ...
    public Component_Movement_Data[] DataWithFrame(int frame)
    {
        if (componentData == null || componentData.Length == 0)
            return null;
        List<Component_Movement_Data> dataAtFrame = new List<Component_Movement_Data>();
    ...
    public void RemoveComponentData(Component_Movement_Data dataToRemove)
    {
        if (componentData == null || componentData.Length == 0)
            return;
...
[Serializable]
public class Component_Movement_Data
{
    ...
    public Component_Movement_Data()
    {
        nickname = "Movement_Data";
        frames = new List<int>();
    ...
    public void CopyToNew(Component_Movement_Data copy)
    {
...
public class Component_Movement_CreateNew : EditorWindow
{
    ...
    string stateName;
    //
    Component_Movement componentSelected;
    //
    Component_Movement_Data newComponentData;
    ...
    [MenuItem("Window/Component Editors/Component_Movement/CreateNew")]
    public static void ShowWindow()
    {
        Component_Movement_CreateNew editorWindow = GetWindow<Component_Movement_CreateNew>();
        editorWindow.Init();
    }
    void Init()
    {
    ...
    void TryLoadComponentAsset()
    {
        ...
        if (componentSelected != null)
            return;
        componentSelected = Assets<Component_Movement>.LoadExistingAsset(controller, stateName);
        if(componentSelected == null)
        {
            GUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            GUILayout.Label($"No {typeof(Component_Movement).Name} exists for state {stateName}", EditorStyles.boldLabel);
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
            if(GUILayout.Button($"CREATE {controller.name}_{stateName}_{typeof(Component_Movement).Name}.asset"))
            {
                componentSelected = Assets<Component_Movement>.CreateNewAsset(controller, stateName);
    ...
    void DataEdit()
    {
        ...
        if(newComponentData == null)
        {
            newComponentData = new Component_Movement_Data();
...
public class Component_Movement_Edit : EditorWindow
{
    GameObject animatorObject;
    Animator animator;
    SpriteRenderer spriteRenderer;
    ...
    Component_Movement componentSelected;
    //
    Component_Movement_Data[] dataAtFrame;
    Component_Movement_Data dataSelected;
    SelectionList<Circle> selectedCircles;
    ListSelectionAndEdit<Circle> circleEdit;
    //
    int dataFilterMode;//Basically ignore frame and show all component data
    bool deleteWarning;
    [MenuItem("Window/Component Editors/Component_Movement/Edit")]
    public static void ShowWindow()
    {
        Component_Movement_Edit editorWindow = GetWindow<Component_Movement_Edit>();
        editorWindow.Init();
    }
    void Init()
    {
    ...
    void ComponentDataSelectAll()
    {
        ...
        if (dataSelected == null)
        {
            ...
            {
                Component_Movement_Data data = componentSelected.componentData[i];
    ...
    void ComponentDataSelectByFrame()
    {
        ...
        if (dataSelected == null)
        {
            ...
            {
                Component_Movement_Data data = dataAtFrame[i];
                if (GUILayout.Button($"Component_Data: {data.nickname} [ Applied Frames: {data.GetAppliedFramesLabel()} ]"))
                {
                    dataSelected = data;
                }
